...
namespace DwTP
{
    ...
    public class dwTP : TerrariaPlugin
    {
        ...
        {
            Commands.ChatCommands.Add(new Command("dw.use", dwCmd, "dw", "定位"));
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                Commands.ChatCommands.RemoveAll(cmd => cmd.CommandDelegate == dwCmd);
        ...
        private void OnNpcKilled(NpcKilledEventArgs e)
        {
            ...
            if (Bag)
            {
                BagX = (int)e.npc.position.X;
                BagY = (int)e.npc.position.Y;
            }
        }
        #endregion
        ...
        private void dwCmd(CommandArgs args)
        {
            ...
            {
                args.Player.SendMessage($"[i:3455][c/AD89D5:定][c/D68ACA:位][c/DF909A:传][c/E5A894:送][i:3454] [C/BFDFEA:by] [c/7CAEDD:羽学][i:3459]\n" +
                    $"/dw wg ——传送到[c/E2E4C4:微光湖]附近\n" +
                    $"/dw sm ——传送到[c/E2E4C4:神庙]附近\n" +
                    $"/dw hb ——传送到[c/E2E4C4:花苞]附近\n" +
                    $"/dw bag ——传送到[c/E2E4C4:宝藏袋]附近\n" +
                    $"/dw dl ——传送到[c/E2E4C4:地牢]附近[骷髅王前/石后]", 247, 244, 150);
            }
            ...
            else if (args.Parameters.Count == 1 && args.Parameters[0] == "wg" || args.Parameters[0] == "微光")
            {
                ...
                if (flag)
                {
                    args.Player.SendMessage($"已将您传送到[c/F25156:微光湖]附近([c/E2E4C4:{x2} {y2}])", 222, 192, 223);
            ...
            else if (args.Parameters.Count == 1 && args.Parameters[0] == "sm" || args.Parameters[0] == "神庙")
            {
                ...
                if (flag)
                {
                    args.Player.SendMessage($"已将您传送到[c/F25156:神庙]附近([c/E2E4C4:{x2} {y2}])", 222, 192, 223);
                    args.Player.Teleport(x2 * 16, y2 * 16, 10);
                }
                return;
            }
            ...
            else if (args.Parameters.Count == 1 && args.Parameters[0] == "hb" || args.Parameters[0] == "花苞")
            {
                ...
                if (flag)
                {
                    args.Player.SendMessage($"已将您传送到[c/F25156:花苞]附近([c/E2E4C4:{x2} {y2}])", 222, 192, 223);
                    args.Player.Teleport(x2 * 16, y2 * 16, 10);
                }
                else
                {
                    args.Player.SendMessage($"世纪之花[c/F25156:未生长],无法获取[c/E2E4C4:花苞坐标]", 222, 192, 223);
                }
                return;
            }
            ...
            else if (args.Parameters.Count == 1 && args.Parameters[0] == "dl" || args.Parameters[0] == "地牢")
            {
                ...
                if (flag)
                {
                    args.Player.SendMessage($"已将您传送到[c/F25156:地牢]附近([c/E2E4C4:{npcX / 16} {npcY / 16}])", 222, 192, 223);
                    args.Player.Teleport(npcX, npcY, 10);
                }
                else
                {
                    args.Player.SendMessage($"地牢守护者[c/F25156:已击败],无法获取[c/E2E4C4:地牢坐标]", 222, 192, 223);
                }
            ...
            else if (args.Parameters.Count == 1 && args.Parameters[0] == "bag" || args.Parameters[0] == "宝藏袋")
            {
                if (Bag)
                {
                    args.Player.SendMessage($"已将您传送到[c/F25156:宝藏袋]附近([c/E2E4C4:{BagX / 16} {BagY / 16}])", 222, 192, 223);
                    args.Player.Teleport(BagX, BagY, 10);
                }
                return;
            }
        }
